<script lang="ts">
	...
	const fetch_contests = async () => {
		const url = 'https://o02lga290c.execute-api.us-east-1.amazonaws.com/dev/posts';
	...
	$: query = createQuery({
		queryKey: [],
		queryFn: () => fetch_contests()
	});
</script>

<div class="px-4 py-1 sm:px-0 ">
	<div class=" relative flex items-center justify-between border-t border-gray-700 mt-5 mb-5" />
	...
	<div class="grid lg:grid-cols-3  md:grid-cols-2 sm:grid-cols-1 gap-3 mt-3">
		...
		{:else}
			{#each $query?.data as contest (contest.id)}
				<div
